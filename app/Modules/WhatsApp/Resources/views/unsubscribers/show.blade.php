<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="page-title">Unsubscriber Import Details</h2>
        </div>
    </x-slot>
            ...
            <div class="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
                <section class="ui-card ui-card-pad">
                    ...
                    <dl class="mt-4 space-y-3 text-sm">
                        ...
                        <div>
                            <dt class="ui-muted">Suppressed</dt>
                            <dd class="ui-strong">{{ number_format($import->successful_rows) }}</dd>
                        </div>
                        <div>
                            <dt class="ui-muted">Already suppressed</dt>
                            <dd class="ui-strong">{{ number_format($import->duplicate_rows) }}</dd>
                        </div>
                        <div>
                        ...
                        @if ($import->error_message)
                            <div>
                                <dt class="ui-muted">Error</dt>
                                <dd class="ui-strong text-red-600 text-xs">{{ $import->error_message }}</dd>
                            </div>
                        @endif
                    </dl>

                    <div class="mt-6">
                        <a href="{{ route('modules.whatsapp.unsubscribers.index') }}" class="ui-button-subtle text-sm">← Back</a>
                    </div>
                </section>
